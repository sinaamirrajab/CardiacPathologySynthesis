...
class Visualizer():
    ...
    def save_images(self, webpage, visuals, image_path,combine=False,separate_folder=None):
        """

        :param webpage:
        :param visuals:
        :param image_path:
        :param combine: whether put the label image on top of the synthetic image or not
        :param separate_folder: to generate a separate folder per image (for 3D images)
        :return:
        """
        visuals = self.convert_visuals_to_numpy(visuals)
        ...
        if combine:
            # combine images
            img_bkg = visuals['synthesized_image']
            if len(img_bkg.shape) == 2:
                img_bkg = np.dstack((img_bkg, img_bkg, img_bkg))
            img = cv.addWeighted(img_bkg, 0.90, visuals['input_label'], 0.10, 0)
            visuals['overlay'] = img

        for label, image_numpy in visuals.items():
            if separate_folder is not None:
                image_name = os.path.join(separate_folder,label, '%s.png' % (name))
            else:
    ...
    def plot_losses(self):
        ...
        assert os.path.exists(self.log_name),'the given log file is not valid'
        ...
        for rows in range(len(s0)):   # remove the row that has ================ Training Loss
            if  '================' not in s0[rows]:
                # print('======== found one! ======== ')
                # del(sl[rows])
                sl +=[s0[rows]]
        dict={}
        columns = int(len(sl[0]))
        ...
        name = os.path.basename(self.log_name).split('.')[0]
        # print(args.log_file.split('/')[-2])
        name2 = self.log_name.split('/')[-2]
        path = os.path.join(self.opt.out_dir,name+ '_'+name2+'.png')
        plt.savefig(path)
        print('saved in:', path)
        plt.close(fig)
